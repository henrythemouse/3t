...
__author__ = 'Robin Friedrich   friedrich@pythonpros.com (heavily modified by GMW'
__version__ = '2.2.2'
...
class BasicDocument:
    ...
    cookies -- try to pass a cookie to ie - GMW 10-31-02
    """
    title = ''
    cgi = None
    bgcolor = None
    ...
    def __str__(self):
        ...
        if self.cgi:
            ...
            s.append('\n<!-- This file generated using HTMLgen -->\n')
        else:
            s.append(DOCTYPE)
            s.append('\n<!-- This file generated xxx using Python HTMLgen module. -->\n')
...
class SimpleDocument(BasicDocument):
    ...
    Cookie support added by GMW
    Multiple and Alternate stylesheets added by GMW
    """
    base = None
    style = None
    stylesheet = None
...
# The following are modifications to this script that allow
# the insertion of a cookie into the header of this document
# Gary M. Witscher 11=27-02
        if self.cgi:
            if self.cookies:
...
##        s.append('\n<!-- This file generated using Python HTMLgen module. -->\n')
    ...
    def html_head(self):
        ...
        # styles modified to accomodate a list of stylesheets GMW
        # all stylesheets are made 'persistant' - no titles -by default
        # sheets containing 'alternate-' will be written as alternate sheets
        if self.stylesheet:
            if type(self.stylesheet)==ListType:
                for thisEl in range(0,len(self.stylesheet)):
                    if "alternate-" in self.stylesheet[thisEl]:
                        loc=self.stylesheet[thisEl].index("alternate-")
                        title=self.stylesheet[thisEl][loc+10:]
                        s.append('\n <LINK rel="alternate stylesheet" href="%s" type="text/css" title="%s">\n' % (self.stylesheet[thisEl],title))
                    else:
                        s.append('\n <LINK rel="stylesheet" href="%s" type="text/css" >\n' % (self.stylesheet[thisEl]))
            else:
                # not a list, so do the original thing
...
class Form:
    ...
    *cgi* is the URL to the CGI processing program.  Input objects
    ...
    def __str__(self):
        ...
        if self.name: s.append(' NAME="%s"' % self.name)
        if self.name: s.append(' Class="%s"' % self.Class)
...
class Input:
    ...
    Keyword Arguments
        ...
        401 strict adds:
        srcImage --  url of the image to use as a submit button.
        alt -- text alternate to image srouce
    """
...
class Select(UserList.UserList):
    ...
    Keyword Arguments:
        ...
        onFocus -- script, which is executed, when the field receives focus
        401 strict adds:
        enclosure -- all iput tags must be inside a enclosure, default is <DIV>
                    Supply the tag name, the <> will be added as will the closing <>.
    """
            ...
            for item, value in self.data:
                if self.zebra:
                    if zrow=='odd':
                        ...
                        zrow='odd'
#                     s.append('<OPTION style=background-color:'+str(self.zebra[zrow]))
                    s.append('<OPTION Class='+zrow+'row') 
                else:
                    s.append('<OPTION')
                if value is not None:
                    s.append(' Value="%s"' % value)
                ## GMW changed so only a exact match selects option
...
#                     s.append('<OPTION>%s\n' % item)
        s.append('</SELECT>\n')
        if self.enclosure:
            s.insert(0,"<"+str(self.enclosure).upper()+">")
            s.append("</"+str(self.enclosure).upper()+">")
...
class Textarea:
    ...
    def __str__(self):
        s = ['<TEXTAREA id="%s" ROWS=%s COLS=%s' % (self.id, self.rows, self.cols)]
        if self.name: s.append(' NAME="%s"' % self.name)
        if self.onBlur: s.append(' onBlur="%s"' % self.onBlur)
        if self.onChange: s.append(' onChange="%s"' % self.onChange)
        if self.onFocus: s.append(' onFocus="%s"' % self.onFocus)
    ...
    def __init__(self, tabletitle='', **kw):
        ...
        for k in kw.keys():
            if self.__dict__.has_key(k):
                ...
                print `k`, "isn't a valid parameter for this class."
        if self.colspan:
            # alternate format by Gary M. Witscher 10-28-00
            self.body = [['&nbsp;']*self.colspan[0]]
        elif self.heading:
            # trying to get around the default *3 above
            self.body = [['&nbsp;']*len(self.heading)]
        ...
        if self.heading:
            ...
            if type(self.heading[0]) == ListType:
                ...
                for i in range(len(self.heading)):
                    middle = middle + '<TH align='+self.heading_align+' valign='+self.heading_valign+' ColSpan=%s%s>' % \
                             (self.colspan[i], \
                              self.get_body_color(self.heading_color,i)) \
        ...
        for row in self.body:
            ...
            for i in range(len(row)):
                if i == 0 :
                    ...
                    ss1 = self.cell_align
                s.append(stmp % (ss1, self.get_row_color(self.body_color,rowCount)))
##                s.append(stmp % (ss1, self.get_body_color(self.body_color,i)))

##                s.append(stmp % (self.get_body_color(self.body_color,i)))
                s.append(str(row[i]))
                s.append('</TD>\n')
            s.append('</TR>\n')
        #close table
    ...
    def get_row_color(self, colors, rowCount):    #GMW
        """Return bgcolor argument for row rowCount
        """
        if colors is not None:
            if len(colors)==2:
                return ' bgcolor="%s"' % colors[rowCount%2]
            else:
                return ' bgcolor-="%s"' % colors[0]
            return ''
        return ''
...
class Image(AbstractTagSingle):
    ...
    attrs = ('src', 'height', 'width', 'alt', 'border', 'align', 'class','id',
             'hspace','vspace', 'lowsrc', 'name', 'style', 'usemap', 'ismap', 'title' )
