...
def index(req,currentCat=0,currentItem=1,action=0):
    ...
    try:
        ...
        kookyData=kooky2.myCookies(req,"","","","",)
        try:
            config=myFunctions.getConfig(req,kookyData['kookyDB'])
        except:
    ...
    # category image and navagation
        ...
        resultTable=mediaTable(resultData,cookieID['kookyID'],config)
    ...
    if itemID=='0': #All_Items
        if categoryName[:3]=='All':
            if searchText: #ok
                ...
                " order by "+config['catSortColumn']+" desc"
...
def catColumnsold(categoryName,itemID,config):
    ...
    for thisCol in qresult:
        if 'timestamp'in thisCol[1]:
            ...
            pass
        elif thisCol[0]==config['catColumn'] and itemID!='0':
            pass
        elif thisCol[3]=="PRI":
            idCol=thisCol[0]
        elif thisCol[0] in config['primaries']:
    ...
    if itemID=='0':
        ...
        for thisCol in range(1, len(colNames)):
            catHeader.append(colNames[thisCol].upper())
        #~ catHeader.insert(0,config['catColumn'].upper())
        catHeader.append(config['catColumn'].upper())
    else:
    ...
    if itemID=='0':
        ...
        #~ colNames.insert(1,config['catColumn'])
        colNames.append(config['catColumn'])
        for thisCol in colNames:
            selectFields=selectFields+config['catTable']+'.'+thisCol+","
        selectFields=selectFields[:-1]
    else:
...
def mediaTable(mediaData,cookieID,config):
    ...
    for thisRecord in mediaData:
        ...
        for thisCol in range(0,len(thisRecord)):
            ...
            # if it's an image write it to disk
                ...
                mediaRow.append(strict401gen.TD(imageLink,colspan=str(columns),align="center",Class=colClass))
            ...
            # if it's not an image, it's text
                ...
                mediaRow.append(strict401gen.TD(text,colspan=str(columns),Class=colClass))
